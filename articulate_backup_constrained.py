...
if __name__ == '__main__':
    ...
    s0 = np.array([5, 0.2, 0, 0])          # initial downright pendulum state
    dt = 0.05                             # discrete time resolution
    T = 10.0                               # total simulation time
    Q = 1e-3 * np.diag([1., 1., 10., 10.])  # state cost matrix
    P = np.diag([100, 100, 1000, 1000])                  # terminal state cost matrix
    ...
    u_max = np.array([0.6, 1.0])                        # control effort bound
    ...
    fd = jax.jit(discretize(articulate_one_trailer, dt))
    ...
    labels_s = (r'$x(t)$', r'$y(t)$', r'$\theta_0{x}(t)$', r'$\theta_0 - \theta_1(t)$')
    labels_u = (r'$v(t)$',r'$s(t)$')
    for i in range(n):
        ax[i].plot(t, s[:, i])
